    ...
    <ul role="list" className="max-h-[25rem] overflow-y-auto mx-2 space-y-1">
      {friends.sort().map((friend) => {
        ...
        return <li key={friend.id}>
            ...
            >{friend.name}
            {unseenMessagesCount>0?<div className='bg-indigo-600 font-medium text-xs text-white w-4 h-4 rounded-full flex justify-center items-center'>{unseenMessagesCount}</div>:null}</a>
        </li>
      })}
    </ul>
  );
